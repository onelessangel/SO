...
struct block_meta *request_space(struct block_meta *last, size_t size)
{
	...
	DIE((void *)block == requested_chunk, "positions are the same when requesting space");
	...
	block->next = NULL;
	block->status = STATUS_FREE;	// STATUS_MAPPED ??
...
struct block_meta *get_free_block(struct block_meta *base, struct block_meta **last, size_t size)
{
	...
	while (curr && !block_is_usable(*last, size)) {
		*last = curr;
		curr = curr->next;
	}
...
{
	struct block_meta *curr = last;
	size_t total_size = curr->size;

	while (curr && curr->status == STATUS_FREE && curr->next->status == STATUS_FREE) {
